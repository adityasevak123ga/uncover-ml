...
def main(yaml_file, partitions):
    ...
    for alg in learn_lst:
        ...
        ddd["output"]["model"] = alg_outdir + f"/{alg['algorithm']}.model"
        ...
        uncoverml.scripts.learn.callback(alg_yaml, partitions)

        model, feature_sets, final_transform = _load_model(ddd["output"]["model"])
...
def super_predict(s_model, learn_lst, alg_model, alg_yaml):
    ...
    img_out = ImageWriter(img_shape, img_bbox, img_crs, 1, "New.tif", 1, band_tags=["Prediction"])
